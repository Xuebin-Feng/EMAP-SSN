...
ssn_install_macos() {
    ...
    echo "Setting up launchers for EMAP-SSN and EMAP-SSN Tools on macOS..."
    echo "Project root: $PROJECT_ROOT"

    chmod +x src/bin/*.sh
    echo "[OK] Configured execution permissions for scripts in src/bin/"

    # Remove exact application bundles generated by the pre-EMAP installer.
    rm -rf "$PROJECT_ROOT/SSN Viewer.app" "$PROJECT_ROOT/SSN Tools.app"

    ssn_create_macos_app_launcher \
        "$PROJECT_ROOT" "EMAP-SSN" "viewer" "ca.utoronto.emapssn"
    ssn_create_macos_app_launcher \
        "$PROJECT_ROOT" "EMAP-SSN Tools" "tools" "ca.utoronto.emapssn.tools"
    ...
    echo "[OK] Created EMAP-SSN.app and EMAP-SSN Tools.app launchers with visible startup terminals."

    echo ""
    echo "To set a custom icon on macOS:"
    echo "  1. Right-click on 'EMAP-SSN.app' or 'EMAP-SSN Tools.app' in Finder and select 'Get Info'."
    ...
    echo ""
    echo "Setup Complete! You can now run EMAP-SSN and EMAP-SSN Tools using the .app launchers in the project root."
}
...
ssn_install_linux() {
    ...
    echo "Setting up launchers for EMAP-SSN and EMAP-SSN Tools on Linux..."
    echo "Project root: $PROJECT_ROOT"
...
Icon=${VIEWER_ICON}
Terminal=false
StartupWMClass=emapssn
Categories=Science;Biology;
EOF
    chmod +x emapssn.desktop

    cat <<EOF > emapssn_tools.desktop
[Desktop Entry]
Type=Application
Name=EMAP-SSN Tools
...
Icon=${TOOL_ICON}
Terminal=false
StartupWMClass=emapssn_tools
Categories=Science;Biology;
EOF
    chmod +x emapssn_tools.desktop

    echo "[OK] Created emapssn.desktop and emapssn_tools.desktop in project root."
    ...
    if [[ "$install_menu" =~ ^[Yy]$ ]]; then
        ...
        rm -f ~/.local/share/applications/SSN_Viewer.desktop \
            ~/.local/share/applications/SSN_Tools.desktop
        cp emapssn.desktop ~/.local/share/applications/
        cp emapssn_tools.desktop ~/.local/share/applications/
    ...
    if [ -d "$HOME/Desktop" ]; then
        ...
        if [[ "$install_desktop" =~ ^[Yy]$ ]]; then
            rm -f "$HOME/Desktop/SSN_Viewer.desktop" "$HOME/Desktop/SSN_Tools.desktop"
            cp emapssn.desktop "$HOME/Desktop/"
            cp emapssn_tools.desktop "$HOME/Desktop/"
            chmod +x "$HOME/Desktop/emapssn.desktop" "$HOME/Desktop/emapssn_tools.desktop"
            echo "[OK] Launchers successfully copied to your Desktop!"
        fi
    fi

    echo ""
    echo "Setup Complete! You can now run EMAP-SSN and EMAP-SSN Tools using the launchers in the project root."
}
