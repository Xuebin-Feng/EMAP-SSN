...
def _sync_alignment_tiled_option(
    device_combo, execution_combo, candidates=None, *, allow_mps=False
):
    """Expose tiled execution when the selected accelerator supports it."""
    candidates = (
        Hardware_Utils.get_available_devices()
        if candidates is None
        else list(candidates)
    ...
    eligible = [
        ...
        and (allow_mps or candidate.backend != "mps")
            ...
            if device_input and execution_input:
                ...
                def update_execution_options(index=None):
                    _sync_alignment_tiled_option(
                        device_combo,
                        execution_combo,
                        allow_mps=script_name == "Align_Similarity_Matrix.py",
                    )
                ...
                if execution_mode == "tiled":
                    ...
                    if selected is not None:
                        eligible = (
                            ...
                            and tiled_accelerator_support(
                                selected.device, require_memory=True
                            )[0]
                            else []
                        )
                    if not eligible:
