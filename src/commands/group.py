...
_GENERATED_CLUSTER_NAME_RE = re.compile(r'^cluster_[1-9]\d*$')
...
def print_help():
    ...
    Validation:
      ...
      group, groups, and clusters. Canonical labels generated by cluster and subcluster
      are also reserved (e.g., cluster_1 and subcluster_1_1). Noncanonical names
      with leading zeros, such as cluster_001, are allowed.
...
def run(viewer, args):
    ...
    for i in range(0, len(args), 2):
        ...
        if name in _RESERVED_GROUP_NAMES:
            ...
            continue
        if _GENERATED_CLUSTER_NAME_RE.fullmatch(name):
