...
REM =========================================================================
REM Local Startup Script for SSN_Config.py (SSN Viewer Entrypoint)
REM Uses a fixed Python environment at C:\venv\Sequence_Similarity_Network_Viewer
REM Can be executed from anywhere (e.g., desktop)
REM =========================================================================
setlocal EnableDelayedExpansion
set "LAUNCH_MODE=%~1"
call :SANITIZE_MANAGED_ENVIRONMENT

:: Set fixed paths for local installation
set "PROJECT_ROOT=E:\OneDrive - University of Toronto\Research Records\Computational Methods\Sequence_Similarity_Network_Viewer"
set "LOCAL_VENV=C:\venv\Sequence_Similarity_Network_Viewer"
...
:: 2. Validate, create, or repair the local virtual environment
...
:CREATE_VENV
echo Creating local virtual environment at %LOCAL_VENV%...
if not exist "%LOCAL_VENV%" mkdir "%LOCAL_VENV%" >nul 2>nul
"!UV_EXE!" venv --clear --python 3.12 "%LOCAL_VENV%"
if !ERRORLEVEL! neq 0 (
...
"!VENV_PYTHON!" src\Install_Dependencies.py --uv-executable "!UV_EXE!" --venv "%LOCAL_VENV%"
set "INSTALL_ERROR=!ERRORLEVEL!"
...
if not exist "%LOCAL_VENV%\Scripts\python.exe" exit /b 1
"%LOCAL_VENV%\Scripts\python.exe" "src\utilities\Single_Instance_Probe.py" viewer >nul 2>nul
exit /b !ERRORLEVEL!
...
"!VENV_PYTHON!" -c "import sys" >nul 2>nul
if !ERRORLEVEL! neq 0 exit /b 10
"!VENV_PYTHON!" src\Install_Dependencies.py --check-only --uv-executable "!UV_EXE!" --venv "%LOCAL_VENV%"
exit /b !ERRORLEVEL!
...
:ACQUIRE_SETUP_LOCK
set "SSN_SETUP_LOCK_ROOT=%PROJECT_ROOT%\Cache_Files\Launcher_State"
