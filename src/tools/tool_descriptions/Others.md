...
| Accelerator Precision **`ACCELERATOR_PRECISION`** | `automatic_32bit` uses IEEE FP32 for small searches and considers validated TF32 only from 4,096 targets. Explicit `bf16` uses BF16 matmul operands with FP32 normalization and postprocessing, requires a capable CUDA/ROCm, XPU, or MPS accelerator, and must pass length-stratified numerical validation. A BF16 search may proceed with a warning when fewer than 3.125% of tested targets change alignment length and every affected selected-mode length and raw score changes by less than 5%; unchanged-length targets retain the 0.01 per-residue score tolerance. The legacy alias `auto` remains accepted. Forced TF32 requires NVIDIA CUDA. |
