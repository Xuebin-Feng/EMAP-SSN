...
| Accelerator Precision **`ACCELERATOR_PRECISION`** | `automatic_32bit` uses IEEE FP32 for small searches and considers validated TF32 only from 4,096 targets. Explicit `bf16` uses BF16 matmul operands with FP32 normalization and postprocessing, requires a capable CUDA/ROCm, XPU, or MPS accelerator, and must pass length-stratified numerical validation. The legacy alias `auto` remains accepted. Forced TF32 requires NVIDIA CUDA. |
